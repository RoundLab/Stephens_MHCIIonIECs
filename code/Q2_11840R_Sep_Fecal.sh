...
SCRATCH=
# Main results directory for core files (table, tree, taxonomy) and analyses. ResultDir is 16S/results/11840R_Sep_Fecal/ for this run.
ResultDir=
...
MANIFEST=
# A metadata map file. Used the file 16S/metadata/map_11840R_Sep_Fecal.txt for this run.
MAP=
...
# Filter poorly sequenced samples. All but 4 of the 8-month old, cohoused fecal samples have less than 1,000 reads. We will thus only further assess the 16 week old separated-by-genotype samples further.
mkdir filt_tables

singularity exec ${ContainerImage} qiime feature-table filter-samples --m-metadata-file $MAP --i-table table_nochim.qza --o-filtered-table filt_tables/table_SepOnly.qza --p-where "AgeAtCollect == '16weeks'"

# Core diveristy and ANCOM analyses

mkdir SepOnly

singularity exec ${ContainerImage} qiime feature-table summarize --i-table filt_tables/table_SepOnly.qza --o-visualization q2_viz/table_SepOnly.qzv

singularity exec ${ContainerImage} qiime diversity core-metrics-phylogenetic --i-table filt_tables/table_SepOnly.qza --output-dir SepOnly/core_div1.1k --p-sampling-depth 1100 --m-metadata-file $MAP --i-phylogeny tree_root.qza

for bdiv in SepOnly/core_div1.1k/weighted_unifrac_distance_matrix.qza SepOnly/core_div1.1k/unweighted_unifrac_distance_matrix.qza SepOnly/core_div1.1k/bray_curtis_distance_matrix.qza SepOnly/core_div1.1k/jaccard_distance_matrix.qza; do singularity exec ${ContainerImage} qiime diversity beta-group-significance --i-distance-matrix $bdiv --o-visualization ${bdiv%_distance_matrix.qza}_GenoSig.qzv --m-metadata-file $MAP --m-metadata-column Genotype  --p-pairwise --p-permutations 9999; done

mkdir SepOnly/ancom

singularity exec ${ContainerImage} qiime composition add-pseudocount --i-table filt_tables/table_SepOnly.qza --o-composition-table SepOnly/ancom/table_pc.qza

singularity exec ${ContainerImage} qiime composition ancom --i-table SepOnly/ancom/table_pc.qza --o-visualization SepOnly/ancom/table_pc_geno.qzv --m-metadata-file $MAP --m-metadata-column Genotype
